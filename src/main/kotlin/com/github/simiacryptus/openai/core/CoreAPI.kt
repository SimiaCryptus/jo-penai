...
import com.github.simiacryptus.openai.StringTools
...
open class CoreAPI(
    ...
    fun getEngines(): Array<CharSequence?> {
        val engines = Companion.mapper.readValue(
            get(apiBase + "/engines"),
...
//        if (apiKey.length == 0) {
//            synchronized(CoreAPI.javaClass) {
//                apiKey = key
//                if (apiKey.length == 0) {
...
//        }
    ...
    fun processCompletionResponse(result: String): CompletionResponse {
        checkError(result)
        val response = Companion.mapper.readValue(
            result,
    ...
    private fun checkError(result: String) {
        try {
            ...
            if (jsonObject.has("error")) {
                ...
                val matcher = Companion.maxTokenErrorMessage.matcher(errorMessage)
                if (matcher.find()) {
    ...
    companion object {
        val log = LoggerFactory.getLogger(CoreAPI::class.java)
        ...
        val completionResponse = try {
            val request: String =
                StringTools.restrictCharacterSet(
                    Companion.mapper.writeValueAsString(completionRequest),
                    Companion.allowedCharset
                )
            val result =
    ...
    private fun logStart(completionRequest: ChatRequest) {
        log(
            logLevel, String.format(
                "Chat Request\nPrefix:\n\t%s\n",
                Companion.mapper.writeValueAsString(completionRequest).replace("\n", "\n\t")
            )
        )
    }
    ...
    fun moderate(text: String) {
        val body: String = try {
            mapper.writeValueAsString(
                Map.of(
                    "input",
                    StringTools.restrictCharacterSet(text, Companion.allowedCharset)
                )
            )
