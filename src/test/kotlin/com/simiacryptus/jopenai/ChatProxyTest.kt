...
class ChatProxyTest {
    ...
    fun testDemo1() {
        ...
        val gptProxy = ChatProxy(TextSummarizer::class.java, api = OpenAIClient(ClientUtil.keyTxt))
    ...
    fun testDemo3() {
        if (ClientUtil.keyTxt.isBlank()) return
        val gptProxy = ChatProxy(RecipeGenerator::class.java, api = OpenAIClient(ClientUtil.keyTxt))
        val generator = gptProxy.create()
    ...
    fun testDemo5() {
        if (ClientUtil.keyTxt.isBlank()) return
        val gptProxy = ChatProxy(WeatherForecast::class.java, api = OpenAIClient(ClientUtil.keyTxt))
